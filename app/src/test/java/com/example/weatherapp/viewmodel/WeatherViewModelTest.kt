...
class WeatherViewModelTest {
    ...
    @Test
    fun `loadLastSearchedCity loads and searches for saved city`() = runTest {
        val savedCity = "Dallas"
        val weather = Weather(savedCity, 25.0, "sunny", "icon")
        
        every { preferencesHelper.getLastCity() } returns savedCity
        coEvery { getWeatherByCityUseCase(savedCity) } returns Result.success(weather)
    ...
    @Test
    fun `loadLastSearchedCity does nothing when no city saved`() = runTest {
        every { preferencesHelper.getLastCity() } returns null
